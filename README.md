...
- `min-password-length <Intger>` - _the minimum length to allow for passwords._
- `min-password-entropy <Integer>` - _the minimum bit entropy in entropy score calculation for passwords._
- `min-password-score <Double>` - _the minimum acceptable password strength score. Should be in the range 0 to 1._
- `password-requires-uppercase <Boolean>` - _whether passwords are required to have an uppercase letter._
- `password-requires-special <Boolean>` - _whether passwords are required to have a special character._
